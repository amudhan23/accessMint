...
import { Coins, Store, Flame, BarChart3, ArrowRightLeft, Zap } from 'lucide-react'
...
export default function App() {
  ...
  const addActivity = (entry) => {
    setActivity(prev => [{ ...entry, id: Date.now(), time: new Date().toLocaleTimeString() }, ...prev])
  }

  const tabs = [
    { id: 'provider', label: 'Provider', icon: Zap, desc: 'Create access plans' },
    { id: 'user', label: 'Dashboard', icon: Coins, desc: 'Buy & use tokens' },
    { id: 'marketplace', label: 'Marketplace', icon: Store, desc: 'Resell tokens' },
  ]
    ...
    <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-sm">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-mint-500/10 text-mint-400 border border-mint-500/20">
      ...
      <footer className="border-t border-gray-800 mt-16">
        <div className="max-w-7xl mx-auto px-6 py-6 text-center text-sm text-gray-600">
          Built at ETHGlobal NYC 2026 · Powered by Hedera Token Service & Consensus Service
        </div>
      </footer>
    </div>
  )
