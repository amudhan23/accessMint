  ...
  const createPlan = async () => {
    ...
    try {
      const res = await fetch('/api/plans', {
        ...
        body: JSON.stringify({ name, symbol, totalSupply: supply, pricePerTokenHbar: price })
      })
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500">Create access plans and mint tokens for your API</p>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="block text-sm text-gray-400 mb-1.5">Plan Name</label>
            <input
              type="text"
              value={name}
          ...
          <div className="col-span-2">
            <label className="block text-sm text-gray-400 mb-1.5">Price per Token (HBAR)</label>
            <input
              type="number"
              step="0.01"
              value={price}
        ...
        <div className="bg-gray-800/50 rounded-xl p-4 text-sm text-gray-400">
          <p>This will mint <span className="text-white font-medium">{supply}</span> access tokens on Hedera using HTS.</p>
          <p>Total revenue at full sale: <span className="text-mint-400 font-medium">{(supply * price).toFixed(2)} HBAR</span></p>
        </div>
            ...
            <div key={i} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{plan.symbol} · Token ID: {plan.tokenId}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="font-semibold text-mint-400">{plan.totalSupply} tokens</p>
                <p className="text-sm text-gray-500">{plan.pricePerTokenHbar} HBAR each</p>
              </div>
            </div>
          ))}
        </div>
