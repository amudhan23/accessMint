...
export default function UserPage({ plans, userTokens, setUserTokens, listings, setListings, addActivity }) {
  ...
  const listForSale = async () => {
    ...
    try {
      ...
      const discount = ((1 - sellPrice / activePlan.pricePerTokenHbar) * 100).toFixed(0)
      addActivity({
        type: 'list',
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-12 text-center">
          ...
          <p className="text-gray-600 text-sm mt-1">Switch to the Provider tab to create one</p>
        </div>
      </div>
    )
  }
      ...
      <div className="bg-gradient-to-br from-gray-900 to-gray-900/50 border border-gray-800 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-4xl font-bold mt-1">
              {balance} <span className="text-lg text-gray-500">{activePlan?.symbol}</span>
            </p>
          </div>
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 bg-gray-800/50 rounded-xl p-3 text-center">
            ...
            <p className="font-mono text-sm mt-0.5 text-mint-400">{activePlan?.tokenId}</p>
          </div>
          <div className="flex-1 bg-gray-800/50 rounded-xl p-3 text-center">
            <p className="text-xs text-gray-500">Retail Price</p>
            <p className="font-medium text-sm mt-0.5">{activePlan?.pricePerTokenHbar} HBAR</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4">
          ...
          <p className="text-xs text-gray-500">
            Cost: <span className="text-white">{(buyAmount * (activePlan?.pricePerTokenHbar || 0)).toFixed(2)} HBAR</span>
          </p>
          <button
            onClick={buyTokens}
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 space-y-4">
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-1">Price per token (HBAR)</label>
            <input
              type="number"
              step="0.01"
              value={sellPrice}
          ...
          {activePlan && sellPrice < activePlan.pricePerTokenHbar && (
            <p className="text-xs text-mint-400">
              {((1 - sellPrice / activePlan.pricePerTokenHbar) * 100).toFixed(0)}% discount vs retail
            </p>
          )}
          <button
            onClick={listForSale}
